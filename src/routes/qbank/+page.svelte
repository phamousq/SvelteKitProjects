...
<main>
	<p>
		Correct: {correctCount} / {incorrectCount + correctCount} or {(
			(correctCount / (incorrectCount + correctCount)) *
			100
		).toFixed(2)}%
	</p>
		...
		{#each Object.entries(currentQuestion['Answer Choices']) as [key, value]}
			<label>
				<input
					...
					disabled={currentQuestionState === 'answered-correct' ||
						currentQuestionState === 'answered-incorrect'}
				/>
				{key}: {value}
			</label>
			<br />
	...
	<button
		...
		disabled={currentQuestionState === 'answered-correct' ||
			currentQuestionState === 'answered-incorrect'}>Check Answer</button
	>
