...
import { curveLinearClosed, lineRadial, line, pointRadial, curveBumpX, curveBumpY, link, stack, stackOffsetExpand, stackOffsetDiverging, stackOffsetNone, arc, pie } from "d3-shape";
...
function Bars($$payload, $$props) {
  ...
  let spring2 = fallback($$props["spring"], void 0);
  let tweened2 = fallback($$props["tweened"], void 0);
  let onbarclick = fallback($$props["onbarclick"], () => {
  });
  _data = chartDataArray(data ?? store_get($$store_subs ??= {}, "$contextData", contextData));
  $$payload.out += `<g class="Bars"><!---->`;
  slot($$payload, $$props, "default", {}, () => {
    const each_array = ensure_array_like(_data);
    $$payload.out += `<!--[-->`;
    for (let i = 0, $$length = each_array.length; i < $$length; i++) {
      let d = each_array[i];
      Bar($$payload, spread_props([
        {
          ...
          x1,
          y1,
          fill: fill ?? (store_get($$store_subs ??= {}, "$config", config).c ? store_get($$store_subs ??= {}, "$cGet", cGet)(d) : null),
          fillOpacity,
          stroke,
          strokeWidth,
          opacity,
...
function BarChart($$payload, $$props) {
  ...
  push();
  var $$store_subs;
  let isVertical, isDefaultSeries, stackSeries, groupSeries, xScale, xBaseline, yScale, yBaseline, visibleSeries, allSeriesData, chartData, getBarsProps;
  let data = fallback($$props["data"], () => [], true);
  let x = fallback($$props["x"], void 0);
  let y = fallback($$props["y"], void 0);
  ...
  let bandPadding = fallback($$props["bandPadding"], 0.4);
  let groupPadding = fallback($$props["groupPadding"], 0);
  let stackPadding = fallback($$props["stackPadding"], 0);
  let tooltipContext2 = fallback($$props["tooltipContext"], void 0);
  ...
  xScale = $$sanitized_props.xScale ?? (isVertical ? scaleBand().padding(bandPadding) : scaleLinear());
  xBaseline = isVertical ? void 0 : 0;
  yScale = $$sanitized_props.yScale ?? (isVertical ? scaleLinear() : scaleBand().padding(bandPadding));
  yBaseline = isVertical ? 0 : void 0;
  ...
  if (seriesLayout === "group") {
    if (isVertical) {
      ...
      x1Range = ({ xScale: xScale2 }) => [0, xScale2.bandwidth?.()];
    } else {
      y1Scale = scaleBand().padding(groupPadding);
      y1Domain = visibleSeries.map((s) => s.key);
      y1Range = ({ yScale: yScale2 }) => [0, yScale2.bandwidth?.()];
    }
  }
  allSeriesData = visibleSeries.flatMap((s) => s.data?.map((d) => {
  ...
  getBarsProps = (s, i) => {
    ...
    const valueAccessor = stackSeries ? (d) => d.stackData[i] : s.value ?? (s.data ? void 0 : s.key);
    const barsProps = {
      data: s.data,
      x: !isVertical ? valueAccessor : void 0,
      y: isVertical ? valueAccessor : void 0,
      x1: isVertical && groupSeries ? (d) => s.value ?? s.key : void 0,
      y1: !isVertical && groupSeries ? (d) => s.value ?? s.key : void 0,
      ...
      ...props.bars,
      ...s.props,
      class: cls("transition-opacity", highlightSeriesKey && highlightSeriesKey !== s.key && "opacity-10", props.bars?.class, s.props?.class)
    };
    return barsProps;
  };
  let $$settled = true;
  ...
  function $$render_inner($$payload2) {
    Chart($$payload2, spread_props([
      {
        data: chartData,
        x: x ?? (stackSeries ? (d) => visibleSeries.flatMap((s, i) => d.stackData[i]) : visibleSeries.map((s) => s.value ?? s.key)),
        xScale,
        xBaseline,
                ...
                asAny(renderContext === "canvas" ? props.canvas : props.svg),
                {
                  ...
                  children: ($$payload4) => {
                    ...
                    slot($$payload4, $$props, "marks", spread_props([{}, slotProps]), () => {
                      ...
                      for (let i = 0, $$length = each_array.length; i < $$length; i++) {
                        let s = each_array[i];
                        Bars($$payload4, spread_props([getBarsProps(s, i)]));
                      }
                      $$payload4.out += `<!--]-->`;
                    });
                    $$payload4.out += `<!----> <!---->`;
                    ...
                    $$payload4.out += `<!----> <!---->`;
                    slot($$payload4, $$props, "highlight", spread_props([{}, slotProps]), () => {
                      Highlight($$payload4, spread_props([{ area: true }, props.highlight]));
                    });
                    $$payload4.out += `<!----> `;
                    if (labels) {
                      $$payload4.out += "<!--[-->";
                      ...
                      for (let i = 0, $$length = each_array_1.length; i < $$length; i++) {
                        ...
                        Labels($$payload4, spread_props([getLabelsProps(s)]));
                      }
                      $$payload4.out += `<!--]-->`;
                    } else {
                      $$payload4.out += "<!--[!-->";
                    ...
                    $$slots: {
                      default: ($$payload4, { data: data2 }) => {
                        TooltipHeader($$payload4, spread_props([
                          {
                            value: isVertical ? x2(data2) : y2(data2),
                            format
                          },
                        ...
                        TooltipList($$payload4, spread_props([
                          props.tooltip?.list,
                          {
                            children: ($$payload5) => {
                              ...
                              const each_array_2 = ensure_array_like(seriesItems);
                              $$payload5.out += `<!--[-->`;
                              for (let $$index_2 = 0, $$length = each_array_2.length; $$index_2 < $$length; $$index_2++) {
...
${rows}`;
  ...
  $$payload.out += `<div><div id="ChartContainer" class="grid gap-4 grid-cols-3 svelte-z8ax2j"><div class="h-[300px] p-4 border rounded col-span-2">`;
  BarChart($$payload, {
    data: lastSevenDaysData(),
    x: "label",
    series: [
      { key: "correct", color: "#D4EDDA" },
      { key: "incorrect", color: "#F8D7DA" }
    ],
    seriesLayout: "stack",
    props: {
      xAxis: { format: "none" },
      ...
      tooltip: {
        ...
        list: {
          format: "none",
          item: { format: "none", value: "This is a test" }
        }
      }
    },
    legend: false,
    $$slots: {
      tooltip: ($$payload2, { x, y }) => {
        {
          $$payload2.out += `<!---->`;
          Tooltip($$payload2, {
            children: invalid_default_snippet,
            $$slots: {
              default: ($$payload3, { data }) => {
                $$payload3.out += `<!---->`;
                TooltipHeader($$payload3, {
                  children: ($$payload4) => {
                    $$payload4.out += `<!---->${escape_html(data.label)}`;
                  },
                  $$slots: { default: true }
                });
                $$payload3.out += `<!----> <!---->`;
                TooltipList($$payload3, {
                  children: ($$payload4) => {
                    $$payload4.out += `<!---->`;
                    TooltipItem($$payload4, { label: "Correct", value: data.correct });
                    $$payload4.out += `<!----> <!---->`;
                    TooltipItem($$payload4, { label: "Incorrect", value: data.incorrect });
                    $$payload4.out += `<!---->`;
                  },
                  $$slots: { default: true }
                });
                $$payload3.out += `<!----> <!---->`;
                TooltipSeparator($$payload3, {});
                $$payload3.out += `<!----> <!---->`;
                TooltipList($$payload3, {
                  children: ($$payload4) => {
                    $$payload4.out += `<!---->`;
  ...
  $$payload.out += `<!----></div> <div id="PieChart" class="h-[300px] p-4 border rounded resize overflow-auto svelte-z8ax2j">`;
  PieChart($$payload, {
  ...
  $$payload.out += `<!----></div></div> <div class="wrapper svelte-z8ax2j" id="QuestionContainer"><div><h1 style="font-size: 32px; font-weight: bold;">Question: ${escape_html(dailyQuestionCount)}</h1></div></div> <div id="SourceInput" class="input-container svelte-z8ax2j" style="padding-top: 10px"><label for="source-input">Source:</label> <input id="source-input" type="text"${attr("value", source)} placeholder="Enter source..." class="source-input svelte-z8ax2j"></div> <div class="input-container svelte-z8ax2j"><label for="source-input">Notes:</label> <input type="text"${attr("value", currentNotes)} placeholder="Enter notes..." class="source-input svelte-z8ax2j" id="notes-input"></div> <div id="Scoreboard" class="scoreboard svelte-z8ax2j"><button class="counter-box correct svelte-z8ax2j" style="flex-grow: 10"><h2>Correct (Enter)</h2> <p>${escape_html(visibleCorrectCount)}</p></button> <button class="counter-box incorrect svelte-z8ax2j" style="flex-grow: 10"><h2>Incorrect (Shift+Enter)</h2> <p>${escape_html(visibleIncorrectCount)}</p></button> <button class="counter-box undo svelte-z8ax2j" style="flex-grow: 1"><h2>Undo (Cmd+z)</h2></button></div>  <div class="date-navigation svelte-z8ax2j">`;
  Button($$payload, {
    class: "max-w-1/3",
  ...
  $$payload.out += `<!----> <div class="current-date svelte-z8ax2j"><button>${escape_html(isToday(currentDate) ? "Today" : formatDate2(currentDate))}</button></div> `;
  Button($$payload, {
    class: "max-w-1/3",
    onclick: goToNextDay,
  ...
  if (filteredHistory.length > 0) {
    $$payload.out += "<!--[-->";
    $$payload.out += `<div class="filtered-stats svelte-z8ax2j"><p>${escape_html(filteredHistory.length)} questions on ${escape_html(formatDate2(currentDate))}: <span class="correct-stat svelte-z8ax2j">${escape_html(visibleCorrectCount)}</span> / <span class="incorrect-stat svelte-z8ax2j">${escape_html(visibleIncorrectCount)}</span> (${escape_html(visiblePercentCorrect)}%)</p></div>`;
  } else {
    $$payload.out += "<!--[!-->";
    $$payload.out += `<div class="filtered-stats empty svelte-z8ax2j"><p>No entries found for ${escape_html(formatDate2(currentDate))}</p></div>`;
  }
  $$payload.out += `<!--]--> <div class="history-table svelte-z8ax2j"><table class="svelte-z8ax2j"><thead><tr class="svelte-z8ax2j"><th class="svelte-z8ax2j">#</th><th class="svelte-z8ax2j">${escape_html(visiblePercentCorrect)}%</th><th class="svelte-z8ax2j">${escape_html(calculateAverageTimeDifference(filteredHistory))}</th><th class="svelte-z8ax2j">Notes</th><th class="svelte-z8ax2j">Source</th></tr></thead><tbody><!--[-->`;
  for (let index = 0, $$length = each_array.length; index < $$length; index++) {
    let item = each_array[index];
    $$payload.out += `<tr class="svelte-z8ax2j"><td class="svelte-z8ax2j">${escape_html(filteredHistory.length - index)}</td><td class="svelte-z8ax2j">${escape_html(item.result)}</td><td class="svelte-z8ax2j">${escape_html(item.timeDifference)}</td><td class="svelte-z8ax2j"><textarea class="svelte-z8ax2j">`;
    const $$body = escape_html(item.notes);
    if ($$body) {
      $$payload.out += `${$$body}`;
    }
    $$payload.out += `</textarea></td><td class="svelte-z8ax2j">${escape_html(item.source || "")}</td></tr>`;
  }
  $$payload.out += `<!--]-->`;
  if (filteredHistory.length === 0) {
    $$payload.out += "<!--[-->";
    $$payload.out += `<tr class="svelte-z8ax2j"><td colspan="5" class="empty-message svelte-z8ax2j">No entries for this date.</td></tr>`;
  } else {
    $$payload.out += "<!--[!-->";
  }
  $$payload.out += `<!--]--></tbody></table></div> <div class="wrapper pb-4 svelte-z8ax2j">`;
  if (history.length > 0) {
    $$payload.out += "<!--[-->";
    Button($$payload, {
      onclick: resetCounts,
