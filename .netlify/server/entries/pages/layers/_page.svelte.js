...
function Icon($$payload, $$props) {
	...
	if (typeof data2 === 'object' && data2 && 'icon' in data2) {
		const [_width, _height, _ligatures, _unicode, _path] = data2.icon;
	...
	if (svg || svgUrl || $$slots.default) {
		...
		$$payload.out += `<span${add_styles(
			merge_styles($$sanitized_props.style, {
				width,
				height,
				'--width': width,
				'--height': height
...
function ProgressCircle($$payload, $$props) {
	...
	$$payload.out += `<div${spread_attributes(
		...
		'svelte-c2dy19',
		{ indeterminate }
	)}><svg xmlns="http://www.w3.org/2000/svg"${attr('style', `transform: rotate(${stringify(rotate - (indeterminate ? 0 : 90))}deg)`)}${attr(
		'viewBox',
...
function Line($$payload, $$props) {
	...
	path =
		'M' +
		store_get(($$store_subs ??= {}), '$data', data2)
			.map((d) => {
				return (
		...
		area =
			...
			('L' +
				store_get(
					...
					store_get(($$store_subs ??= {}), '$extents', extents).x
