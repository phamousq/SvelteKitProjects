import {
	$ as noop,
	ag as current_component,
	a8 as sanitize_props,
	ad as rest_props,
	a0 as getContext,
	a2 as fallback,
	ac as spread_attributes,
	ab as clsx,
	a4 as slot,
	ae as element,
	a6 as bind_props,
	...
	_ as stringify,
	a3 as store_get,
	a5 as unsubscribe_stores,
	ah as store_set,
	S as setContext,
	X as add_styles,
	a7 as sanitize_slots,
	ai as invalid_default_snippet,
	af as spread_props,
	V as ensure_array_like,
	aj as copy_payload,
	ak as assign_payload
...
function Progressbar($$payload, $$props) {
	...
	let labelInsideClass = fallback(
		$$props['labelInsideClass'],
...
function ChartContext($$payload, $$props) {
	...
	store_set(
		...
		x1Domain ??
			extent(
				chartDataArray(store_get(($$store_subs ??= {}), '$contextData', contextData)),
				store_get(($$store_subs ??= {}), '$_x1', _x1)
			)
	);
	store_set(
		_x1Scale,
		x1Scale && x1Range
			? createScale(x1Scale, store_get(($$store_subs ??= {}), '$_x1Domain', _x1Domain), x1Range, {
	...
	store_set(_x1Get, (d) =>
		store_get(
			...
			'$_x1Scale',
			_x1Scale
		)?.(store_get(($$store_subs ??= {}), '$_x1', _x1)(d))
	);
	...
	store_set(
		_y1Scale,
		y1Scale && y1Range
			? createScale(y1Scale, store_get(($$store_subs ??= {}), '$_y1Domain', _y1Domain), y1Range, {
	...
	store_set(_y1Get, (d) =>
		store_get(
			...
			'$_y1Scale',
			_y1Scale
		)?.(store_get(($$store_subs ??= {}), '$_y1', _y1)(d))
	);
	...
	store_set(
		...
		cDomain ??
			unique(
				chartDataArray(store_get(($$store_subs ??= {}), '$contextData', contextData)).map(
					store_get(($$store_subs ??= {}), '$_c', _c)
				)
			)
	);
	...
	store_set(
		...
		cRange
			? createScale(
					cScale ?? scaleOrdinal(),
					store_get(($$store_subs ??= {}), '$_cDomain', _cDomain),
					cRange,
					{
						$width: store_get(($$store_subs ??= {}), '$width', width),
						$height: store_get(($$store_subs ??= {}), '$height', height)
...
function TransformContext($$payload, $$props) {
	...
	const translate = motionStore(initialTranslate ?? DEFAULT_TRANSLATE, {
		spring: spring2,
		tweened: tweened2
	});
		...
		scaleTo(1.25, {
			x:
				(store_get(($$store_subs ??= {}), '$width', width) +
					store_get(($$store_subs ??= {}), '$padding', padding).left) /
				2,
			y:
				(store_get(($$store_subs ??= {}), '$height', height) +
					store_get(($$store_subs ??= {}), '$padding', padding).top) /
				2
		});
	}
	function zoomOut() {
	...
	function zoomTo(center2, rect) {
		const newScale = rect
			? store_get(($$store_subs ??= {}), '$width', width) <
				store_get(($$store_subs ??= {}), '$height', height)
				? store_get(($$store_subs ??= {}), '$width', width) / rect.width
				: store_get(($$store_subs ??= {}), '$height', height) / rect.height
			: 1;
		store_set(translate, {
	...
	function scaleTo(value, point, options = void 0) {
		...
		const invertTransformPoint = {
			x:
				(point.x -
					store_get(($$store_subs ??= {}), '$padding', padding).left -
					store_get(($$store_subs ??= {}), '$translate', translate).x) /
				currentScale,
			y:
				(point.y -
					store_get(($$store_subs ??= {}), '$padding', padding).top -
					store_get(($$store_subs ??= {}), '$translate', translate).y) /
				currentScale
		};
		const newTranslate = {
			x:
				point.x -
				store_get(($$store_subs ??= {}), '$padding', padding).left -
				invertTransformPoint.x * newScale,
			y:
				point.y -
				store_get(($$store_subs ??= {}), '$padding', padding).top -
				invertTransformPoint.y * newScale
		};
	...
	const moving = derived(
		...
		([dragging2, translating2, scaling2]) => dragging2 || translating2 || scaling2
	);
...
function Canvas($$payload, $$props) {
	...
	function update() {
		...
		scaleCanvas(
			context,
			store_get(($$store_subs ??= {}), '$containerWidth', containerWidth),
			store_get(($$store_subs ??= {}), '$containerHeight', containerHeight)
		);
		context.clearRect(
			0,
			0,
		...
		context.translate(
			store_get(($$store_subs ??= {}), '$padding', padding).left ?? 0,
		...
		if (center) {
			const newTranslate = {
				x:
					center === 'x' || center === true
						...
						: 0,
				y:
					center === 'y' || center === true
						? store_get(($$store_subs ??= {}), '$height', height) / 2
						: 0
			};
			...
			context.scale(
				store_get(($$store_subs ??= {}), '$scale', scale),
				store_get(($$store_subs ??= {}), '$scale', scale)
			);
		}
	...
	setRenderContext('canvas');
	{
		store_get(($$store_subs ??= {}), '$containerWidth', containerWidth),
			store_get(($$store_subs ??= {}), '$containerHeight', containerHeight) &&
				store_get(($$store_subs ??= {}), '$dragging', dragging);
	...
	$$payload.out += `<!----></canvas> <canvas${attr(
		'class',
		clsx(
			cls(
...
function Rect($$payload, $$props) {
	...
	let tweened_x = motionStore(
		initialX,
		resolveOptions('x', { spring: spring2, tweened: tweened2 })
	);
	let tweened_y = motionStore(
		initialY,
		resolveOptions('y', { spring: spring2, tweened: tweened2 })
	);
	let tweened_width = motionStore(
		initialWidth,
		resolveOptions('width', { spring: spring2, tweened: tweened2 })
	);
	let tweened_height = motionStore(
		initialHeight,
		resolveOptions('height', { spring: spring2, tweened: tweened2 })
	);
	...
	if (renderContext === 'canvas') {
		store_get(($$store_subs ??= {}), '$tweened_x', tweened_x) &&
			store_get(($$store_subs ??= {}), '$tweened_y', tweened_y) &&
			store_get(($$store_subs ??= {}), '$tweened_width', tweened_width) &&
			store_get(($$store_subs ??= {}), '$tweened_height', tweened_height) &&
			fillKey &&
			strokeKey &&
			strokeWidth &&
			className;
...
function ChartClipPath($$payload, $$props) {
	...
	RectClipPath(
		...
		spread_props([
			{
				x:
					full && store_get(($$store_subs ??= {}), '$padding', padding).left
						? -store_get(($$store_subs ??= {}), '$padding', padding).left
						: 0,
				y:
					full && store_get(($$store_subs ??= {}), '$padding', padding).top
						? -store_get(($$store_subs ??= {}), '$padding', padding).top
						: 0,
				width:
					store_get(($$store_subs ??= {}), '$width', width) +
					(full
						? (store_get(($$store_subs ??= {}), '$padding', padding)?.left ?? 0) +
							(store_get(($$store_subs ??= {}), '$padding', padding)?.right ?? 0)
						: 0),
				height:
					store_get(($$store_subs ??= {}), '$height', height) +
					(full
						? (store_get(($$store_subs ??= {}), '$padding', padding)?.top ?? 0) +
							(store_get(($$store_subs ??= {}), '$padding', padding)?.bottom ?? 0)
						: 0),
				disabled
			},
			$$restProps,
...
function GeoPath($$payload, $$props) {
	...
	_projection = geoTransform$1
		? geoTransform(geoTransform$1(store_get(($$store_subs ??= {}), '$geo', geo)))
		: store_get(($$store_subs ??= {}), '$geo', geo);
...
function Group($$payload, $$props) {
	...
	function render2(ctx) {
		ctx.translate(
			store_get(($$store_subs ??= {}), '$tweened_x', tweened_x) ?? 0,
			store_get(($$store_subs ??= {}), '$tweened_y', tweened_y) ?? 0
		);
	}
	let canvasUnregister;
	onDestroy(() => {
...
function Marker($$payload, $$props) {
	...
	let refX = fallback(
		$$props['refX'],
		() => (['arrow', 'triangle'].includes(type ?? '') ? 9 : 5),
		true
	);
...
function Spline($$payload, $$props) {
	...
	function defaultPathData() {
		if (!tweenedOptions) {
			...
			return flattenPathData(
				pathData,
				Math.min(
					store_get(($$store_subs ??= {}), '$yScale', yScale)(0),
					store_get(($$store_subs ??= {}), '$yRange', yRange)[0]
				)
			);
		} else if (store_get(($$store_subs ??= {}), '$config', config).x) {
			const path2 = store_get(($$store_subs ??= {}), '$radial', radial)
				? lineRadial()
						.angle((d2) => store_get(($$store_subs ??= {}), '$xScale', xScale)(xAccessor(d2)))
						.radius((d2) =>
							Math.min(
								store_get(($$store_subs ??= {}), '$yScale', yScale)(0),
								store_get(($$store_subs ??= {}), '$yRange', yRange)[0]
				...
				: line()
						.x((d2) => store_get(($$store_subs ??= {}), '$xScale', xScale)(xAccessor(d2)) + xOffset)
						.y((d2) =>
							Math.min(
								store_get(($$store_subs ??= {}), '$yScale', yScale)(0),
								store_get(($$store_subs ??= {}), '$yRange', yRange)[0]
	...
	xOffset = isScaleBand(store_get(($$store_subs ??= {}), '$xScale', xScale))
		? store_get(($$store_subs ??= {}), '$xScale', xScale).bandwidth() / 2
		: 0;
	yOffset = isScaleBand(store_get(($$store_subs ??= {}), '$yScale', yScale))
	...
	tweened_d = motionStore(defaultPathData(), { tweened: tweenedOptions });
	{
		const path2 = store_get(($$store_subs ??= {}), '$radial', radial)
			? lineRadial()
					.angle((d2) =>
						getScaleValue(d2, store_get(($$store_subs ??= {}), '$xScale', xScale), xAccessor)
					)
					.radius((d2) =>
						getScaleValue(d2, store_get(($$store_subs ??= {}), '$yScale', yScale), yAccessor)
					)
			: line()
					.x(
						(d2) =>
		...
		d =
			pathData ??
			path2(data ?? store_get(($$store_subs ??= {}), '$contextData', contextData)) ??
			'';
		tweened_d.set(d);
	}
	if (draw) {
			...
			$$payload.out += `<path${spread_attributes(
				{
					...
					...$$restProps,
					class: clsx(
						cls('path-line', !fill && 'fill-none', !stroke && 'stroke-surface-content', className)
					),
					fill,
					stroke,
					'stroke-width': strokeWidth,
...
function Voronoi($$payload, $$props) {
	...
	points = (data ?? store_get(($$store_subs ??= {}), '$flatData', flatData)).map((d) => {
		const xValue = store_get(($$store_subs ??= {}), '$geo', geo)
			? store_get(($$store_subs ??= {}), '$xContext', xContext)(d)
			: store_get(($$store_subs ??= {}), '$xGet', xGet)(d);
		const yValue = store_get(($$store_subs ??= {}), '$geo', geo)
			? store_get(($$store_subs ??= {}), '$yContext', yContext)(d)
...
function TooltipContext($$payload, $$props) {
		...
		switch (findTooltipData) {
			case 'closest':
				if (currentValue === void 0) {
					...
					return Number(valueAtPoint) - Number(accessor2(previousValue)) >
						Number(accessor2(currentValue)) - Number(valueAtPoint)
						? currentValue
						: previousValue;
				}
			case 'left':
			...
			switch (mode) {
				case 'bisect-x': {
					...
					if (store_get(($$store_subs ??= {}), '$radial', radial)) {
						const { radians } = cartesianToPolar(
							point.x - store_get(($$store_subs ??= {}), '$width', width) / 2,
							point.y - store_get(($$store_subs ??= {}), '$height', height) / 2
						);
						xValueAtPoint = scaleInvert(
							store_get(($$store_subs ??= {}), '$xScale', xScale),
							radians
						);
					} else {
						xValueAtPoint = scaleInvert(
							store_get(($$store_subs ??= {}), '$xScale', xScale),
							point.x - store_get(($$store_subs ??= {}), '$padding', padding).left
						);
					}
					const index = bisectX(
						store_get(($$store_subs ??= {}), '$flatData', flatData),
						xValueAtPoint,
						1
					);
					...
					tooltipData = findData(
						previousValue,
						currentValue,
						xValueAtPoint,
						store_get(($$store_subs ??= {}), '$x', x)
					);
					break;
				}
				case 'bisect-y': {
					const yValueAtPoint = scaleInvert(
						store_get(($$store_subs ??= {}), '$yScale', yScale),
						point.y - store_get(($$store_subs ??= {}), '$padding', padding).top
					);
					const index = bisectY(
						store_get(($$store_subs ??= {}), '$flatData', flatData),
						yValueAtPoint,
				...
				case 'bisect-band': {
					...
					if (isScaleBand(store_get(($$store_subs ??= {}), '$xScale', xScale))) {
						const bandData = store_get(($$store_subs ??= {}), '$flatData', flatData)
							.filter((d) => store_get(($$store_subs ??= {}), '$x', x)(d) === xValueAtPoint)
							.sort(sortFunc(store_get(($$store_subs ??= {}), '$y', y)));
						...
						const bandData = store_get(($$store_subs ??= {}), '$flatData', flatData)
							.filter((d) => store_get(($$store_subs ??= {}), '$y', y)(d) === yValueAtPoint)
							.sort(sortFunc(store_get(($$store_subs ??= {}), '$x', x)));
	...
	if (mode === 'bounds' || mode === 'band') {
		rects = store_get(($$store_subs ??= {}), '$flatData', flatData)
			.map((d) => {
				...
				const xOffset = isScaleBand(store_get(($$store_subs ??= {}), '$xScale', xScale))
					? (store_get(($$store_subs ??= {}), '$xScale', xScale).padding() *
							store_get(($$store_subs ??= {}), '$xScale', xScale).step()) /
						2
					: 0;
				const yOffset = isScaleBand(store_get(($$store_subs ??= {}), '$yScale', yScale))
					? (store_get(($$store_subs ??= {}), '$yScale', yScale).padding() *
							store_get(($$store_subs ??= {}), '$yScale', yScale).step()) /
						2
					: 0;
				const fullWidth =
					max(store_get(($$store_subs ??= {}), '$xRange', xRange)) -
					min(store_get(($$store_subs ??= {}), '$xRange', xRange));
				const fullHeight =
					max(store_get(($$store_subs ??= {}), '$yRange', yRange)) -
					min(store_get(($$store_subs ??= {}), '$yRange', yRange));
				if (mode === 'band') {
					return {
						x: isScaleBand(store_get(($$store_subs ??= {}), '$xScale', xScale))
							? x2 - xOffset
							: min(store_get(($$store_subs ??= {}), '$xRange', xRange)),
						y: isScaleBand(store_get(($$store_subs ??= {}), '$yScale', yScale))
							? y2 - yOffset
							: min(store_get(($$store_subs ??= {}), '$yRange', yRange)),
						width: isScaleBand(store_get(($$store_subs ??= {}), '$xScale', xScale))
							? store_get(($$store_subs ??= {}), '$xScale', xScale).step()
							: fullWidth,
						height: isScaleBand(store_get(($$store_subs ??= {}), '$yScale', yScale))
							? store_get(($$store_subs ??= {}), '$yScale', yScale).step()
							: fullHeight,
						data: d
					};
					...
					return {
						x:
							isScaleBand(store_get(($$store_subs ??= {}), '$xScale', xScale)) ||
						...
						width: Array.isArray(xValue)
							...
							: isScaleBand(store_get(($$store_subs ??= {}), '$xScale', xScale))
								? store_get(($$store_subs ??= {}), '$xScale', xScale).step()
								: min(store_get(($$store_subs ??= {}), '$xRange', xRange)) + x2,
						height: Array.isArray(yValue)
							? yValue[1] - yValue[0]
							: isScaleBand(store_get(($$store_subs ??= {}), '$yScale', yScale))
								? store_get(($$store_subs ??= {}), '$yScale', yScale).step()
								: // @ts-expect-error
...
function Area($$payload, $$props) {
	...
	function defaultPathData() {
		const path2 = store_get(($$store_subs ??= {}), '$radial', radial)
			? areaRadial()
					.angle((d) => store_get(($$store_subs ??= {}), '$xScale', xScale)(xAccessor(d)))
					.innerRadius((d) =>
						Math.min(
							store_get(($$store_subs ??= {}), '$yScale', yScale)(0),
							store_get(($$store_subs ??= {}), '$yRange', yRange)[0]
			...
			: area()
					.x((d) => store_get(($$store_subs ??= {}), '$xScale', xScale)(xAccessor(d)) + xOffset)
					.y0((d) =>
						Math.min(
							store_get(($$store_subs ??= {}), '$yScale', yScale)(0),
							store_get(($$store_subs ??= {}), '$yRange', yRange)[0]
	...
	y0Accessor = y0
		? accessor(y0)
		: (d) => min(store_get(($$store_subs ??= {}), '$yDomain', yDomain));
	...
	tweened_d = motionStore(defaultPathData(), { tweened: tweenedOptions });
	{
		const path2 = store_get(($$store_subs ??= {}), '$radial', radial)
			? areaRadial()
					.angle((d2) => store_get(($$store_subs ??= {}), '$xScale', xScale)(xAccessor(d2)))
					.innerRadius((d2) => store_get(($$store_subs ??= {}), '$yScale', yScale)(y0Accessor(d2)))
					.outerRadius((d2) => store_get(($$store_subs ??= {}), '$yScale', yScale)(y1Accessor(d2)))
			: area()
					.x((d2) => store_get(($$store_subs ??= {}), '$xScale', xScale)(xAccessor(d2)) + xOffset)
					.y0((d2) => {
						...
						if (y0) {
							...
							Array.isArray(store_get(($$store_subs ??= {}), '$config', config).y) &&
							store_get(($$store_subs ??= {}), '$config', config).y[0] === 0
						) {
							value = store_get(
								($$store_subs ??= {}),
								'$yScale',
								yScale
							)(store_get(($$store_subs ??= {}), '$y', y)(d2)[0]);
						}
					...
					.y1((d2) => {
						...
						if (y1) {
							...
							Array.isArray(store_get(($$store_subs ??= {}), '$config', config).y) &&
							store_get(($$store_subs ??= {}), '$config', config).y[1] === 1
						) {
							value = store_get(
								($$store_subs ??= {}),
								'$yScale',
								yScale
							)(store_get(($$store_subs ??= {}), '$y', y)(d2)[1]);
						} else {
							value = store_get(
								($$store_subs ??= {}),
								'$yScale',
								yScale
							)(store_get(($$store_subs ??= {}), '$y', y)(d2));
						}
						return value + yOffset;
					});
		...
		if (curve) path2.curve(curve);
		const d =
			pathData ?? path2(data ?? store_get(($$store_subs ??= {}), '$contextData', contextData));
		tweened_d.set(d ?? '');
	}
	fill && typeof fill === 'object' ? objectId(fill) : fill;
	stroke && typeof stroke === 'object' ? objectId(stroke) : stroke;
...
function Line($$payload, $$props) {
	...
	strokeKey = stroke && typeof stroke === 'object' ? objectId(stroke) : stroke;
	if (renderContext === 'canvas') {
		store_get(($$store_subs ??= {}), '$tweened_x1', tweened_x1) &&
			store_get(($$store_subs ??= {}), '$tweened_y1', tweened_y1) &&
			store_get(($$store_subs ??= {}), '$tweened_x2', tweened_x2) &&
			store_get(($$store_subs ??= {}), '$tweened_y2', tweened_y2) &&
			fillKey &&
			strokeKey &&
			strokeWidth &&
			className;
...
function Circle($$payload, $$props) {
	...
	strokeKey = stroke && typeof stroke === 'object' ? objectId(stroke) : stroke;
	if (renderContext === 'canvas') {
		store_get(($$store_subs ??= {}), '$tweened_cx', tweened_cx) &&
			store_get(($$store_subs ??= {}), '$tweened_cy', tweened_cy) &&
			store_get(($$store_subs ??= {}), '$tweened_r', tweened_r) &&
			fillKey &&
			fillOpacity &&
			strokeKey &&
			strokeWidth &&
	...
	showRule = (value, axis) => {
		switch (typeof value) {
			...
			default:
				if (axis === 'x') {
					return (
						store_get(($$store_subs ??= {}), '$xScale', xScale)(value) >= xRangeMin &&
						store_get(($$store_subs ??= {}), '$xScale', xScale)(value) <= xRangeMax
					);
				} else {
					return (
						store_get(($$store_subs ??= {}), '$yScale', yScale)(value) >= yRangeMin &&
						store_get(($$store_subs ??= {}), '$yScale', yScale)(value) <= yRangeMax
					);
				}
		}
	};
	...
	if (showRule(x, 'x')) {
		...
		const xCoord =
			x === true || x === 'left'
				...
				: x === 'right'
					? xRangeMax
					: store_get(($$store_subs ??= {}), '$xScale', xScale)(x) + xOffset;
		if (store_get(($$store_subs ??= {}), '$radial', radial)) {
			$$payload.out += '<!--[-->';
			...
			Circle($$payload, {
				r:
					y === true || y === 'bottom'
						...
						: y === 'top'
							? yRangeMin
							: store_get(($$store_subs ??= {}), '$yScale', yScale)(y) + yOffset,
	...
	function render2(ctx, styleOverrides) {
		wordsByLines.forEach((line2, index) => {
			renderText(
				...
				{
					x:
						getPixelValue(store_get(($$store_subs ??= {}), '$tweened_x', tweened_x)) +
						getPixelValue(dx),
					y:
						getPixelValue(store_get(($$store_subs ??= {}), '$tweened_y', tweened_y)) +
						getPixelValue(dy) +
...
function Axis($$payload, $$props) {
	...
	let transitionInParams = fallback(
		$$props['transitionInParams'],
		() => ({ easing: cubicIn }),
		true
	);
	let scale = fallback($$props['scale'], void 0);
	...
	_scale =
		scale ??
		(['horizontal', 'angle'].includes(orientation)
			? store_get(($$store_subs ??= {}), '$xScale', xScale)
			: store_get(($$store_subs ??= {}), '$yScale', yScale));
	[xRangeMin, xRangeMax] = extent(store_get(($$store_subs ??= {}), '$xRange', xRange));
	[yRangeMin, yRangeMax] = extent(store_get(($$store_subs ??= {}), '$yRange', yRange));
	tickVals = Array.isArray(ticks)
		...
		: typeof ticks === 'function'
			...
			: isLiteralObject(ticks)
				? _scale.ticks(ticks.interval)
				: isScaleBand(_scale)
					? ticks
						? _scale.domain().filter((v, i) => i % ticks === 0)
						: _scale.domain()
					: _scale.ticks(ticks ?? (placement === 'left' || placement === 'right' ? 4 : void 0));
	resolvedLabelProps = {
		value: label,
		x:
			placement === 'left' || (orientation === 'horizontal' && labelPlacement === 'start')
				...
				: placement === 'right' || (orientation === 'horizontal' && labelPlacement === 'end')
					? store_get(($$store_subs ??= {}), '$width', width) +
						store_get(($$store_subs ??= {}), '$padding', padding).right
					: store_get(($$store_subs ??= {}), '$width', width) / 2,
		y:
			placement === 'top' || (orientation === 'vertical' && labelPlacement === 'start')
				...
				: orientation === 'vertical' && labelPlacement === 'middle'
					...
					: placement === 'bottom' || labelPlacement === 'end'
						? store_get(($$store_subs ??= {}), '$height', height) +
							store_get(($$store_subs ??= {}), '$padding', padding).bottom
						: 0,
		textAnchor:
...
function Grid($$payload, $$props) {
	...
	let yTicks = fallback(
		$$props['yTicks'],
		() => (!isScaleBand(store_get(($$store_subs ??= {}), '$yScale', yScale)) ? 4 : void 0),
		true
	);
	...
	xBandOffset = isScaleBand(store_get(($$store_subs ??= {}), '$xScale', xScale))
		? bandAlign === 'between'
			? -(
					store_get(($$store_subs ??= {}), '$xScale', xScale).padding() *
					store_get(($$store_subs ??= {}), '$xScale', xScale).step()
			...
			: store_get(
				...
				(store_get(($$store_subs ??= {}), '$xScale', xScale).padding() *
					...
					2
		: 0;
	yBandOffset = isScaleBand(store_get(($$store_subs ??= {}), '$yScale', yScale))
		? bandAlign === 'between'
			? -(
					store_get(($$store_subs ??= {}), '$yScale', yScale).padding() *
					store_get(($$store_subs ??= {}), '$yScale', yScale).step()
	...
	if (x) {
		...
		if (
			isScaleBand(store_get(($$store_subs ??= {}), '$xScale', xScale)) &&
			bandAlign === 'between' &&
			!store_get(($$store_subs ??= {}), '$radial', radial) &&
			xTickVals.length
		) {
			$$payload.out += '<!--[-->';
			Rule(
...
function Bar($$payload, $$props) {
	...
	let x = fallback(
		$$props['x'],
		() => store_get(($$store_subs ??= {}), '$xContext', xContext),
		true
	);
	let y = fallback(
		$$props['y'],
		() => store_get(($$store_subs ??= {}), '$yContext', yContext),
		true
	);
	let x1 = fallback($$props['x1'], void 0);
	let y1 = fallback($$props['y1'], void 0);
	...
	dimensions = store_get(($$store_subs ??= {}), '$getDimensions', getDimensions)(bar) ?? {
		x: 0,
		y: 0,
		width: 0,
		height: 0
...
function Highlight($$payload, $$props) {
	...
	if (highlightData) {
		...
		const xCoord = Array.isArray(xValue)
			? xValue.map((v) => store_get(($$store_subs ??= {}), '$xScale', xScale)(v))
			: store_get(($$store_subs ??= {}), '$xScale', xScale)(xValue);
		const xOffset =
			isScaleBand(store_get(($$store_subs ??= {}), '$xScale', xScale)) &&
			!store_get(($$store_subs ??= {}), '$radial', radial)
				? store_get(($$store_subs ??= {}), '$xScale', xScale).bandwidth() / 2
		...
		const yCoord = Array.isArray(yValue)
			? yValue.map((v) => store_get(($$store_subs ??= {}), '$yScale', yScale)(v))
			: store_get(($$store_subs ??= {}), '$yScale', yScale)(yValue);
		const yOffset =
			isScaleBand(store_get(($$store_subs ??= {}), '$yScale', yScale)) &&
			!store_get(($$store_subs ??= {}), '$radial', radial)
				? store_get(($$store_subs ??= {}), '$yScale', yScale).bandwidth() / 2
				: 0;
		_lines = [];
		const defaultAxis = isScaleBand(store_get(($$store_subs ??= {}), '$yScale', yScale))
			? 'y'
			: 'x';
		if (axis == null) {
			axis = defaultAxis;
		...
		if (axis === 'x' || axis === 'both') {
			...
			if (Array.isArray(xCoord)) {
				...
				const index = store_get(($$store_subs ??= {}), '$flatData', flatData).findIndex(
					(d) => Number(_x(d)) === Number(_x(highlightData))
				);
				const isLastPoint =
					index + 1 === store_get(($$store_subs ??= {}), '$flatData', flatData).length;
				const nextDataPoint = isLastPoint
					? max(store_get(($$store_subs ??= {}), '$xDomain', xDomain))
					: _x(store_get(($$store_subs ??= {}), '$flatData', flatData)[index + 1]);
				_area.width =
					(store_get(($$store_subs ??= {}), '$xScale', xScale)(nextDataPoint) ?? 0) - (xCoord ?? 0);
			}
			_area.x =
		...
		if (axis === 'y' || axis === 'both') {
			...
			if (Array.isArray(yCoord)) {
				...
				const isLastPoint =
					index + 1 === store_get(($$store_subs ??= {}), '$flatData', flatData).length;
				const nextDataPoint = isLastPoint
					? max(store_get(($$store_subs ??= {}), '$yDomain', yDomain))
					: _x(store_get(($$store_subs ??= {}), '$flatData', flatData)[index + 1]);
				_area.height =
					(store_get(($$store_subs ??= {}), '$yScale', yScale)(nextDataPoint) ?? 0) - (yCoord ?? 0);
			}
			_area.y =
		...
		if (Array.isArray(xCoord)) {
			if (Array.isArray(highlightData)) {
				...
				if (Array.isArray(store_get(($$store_subs ??= {}), '$contextData', contextData))) {
					const seriesPointsData = store_get(($$store_subs ??= {}), '$contextData', contextData)
						.map((series) => {
							return {
								series,
					...
					_points = seriesPointsData.map((seriesPoint, i) => {
						return {
							x:
								store_get(($$store_subs ??= {}), '$xScale', xScale)(seriesPoint.point[1]) + xOffset,
							y: yCoord + yOffset,
							fill: store_get(($$store_subs ??= {}), '$config', config).c
								? store_get(($$store_subs ??= {}), '$cGet', cGet)(seriesPoint.series)
								: null,
				...
				_points = xCoord.filter(notNull).map((xItem, i) => {
					...
					return {
						...
						fill: store_get(($$store_subs ??= {}), '$config', config).c
							? store_get(($$store_subs ??= {}), '$cGet', cGet)({ ...highlightData, $key })
							: null,
						data: {
							x: xValue,
			...
			_points = [
				{
					...
					y: yCoord + yOffset,
					fill: store_get(($$store_subs ??= {}), '$config', config).c
						? store_get(($$store_subs ??= {}), '$cGet', cGet)(highlightData)
						: null,
		...
		.flatMap((d) => {
			...
			if (Array.isArray(xValue)) {
				return xValue.filter(notNull).map((xValue2) => {
					return {
						x:
							store_get(($$store_subs ??= {}), '$xScale', xScale)(xValue2) +
							getOffset(
								store_get(($$store_subs ??= {}), '$xScale', xScale)(xValue2),
								offsetX,
								store_get(($$store_subs ??= {}), '$xScale', xScale)
						...
						r: store_get(($$store_subs ??= {}), '$config', config).r
							? store_get(($$store_subs ??= {}), '$rGet', rGet)(d)
							: r,
						xValue: xValue2,
						yValue,
						data: d
				...
				return yValue.filter(notNull).map((yValue2) => {
					return {
						...
						y:
							store_get(($$store_subs ??= {}), '$yScale', yScale)(yValue2) +
							getOffset(
								store_get(($$store_subs ??= {}), '$yScale', yScale)(yValue2),
								offsetY,
								store_get(($$store_subs ??= {}), '$yScale', yScale)
		...
		if (Array.isArray(xValue)) {
			...
			return {
				source: {
					x:
						xMin +
						getOffset(xMin, offsetX, store_get(($$store_subs ??= {}), '$xScale', xScale)) +
						(store_get(($$store_subs ??= {}), '$config', config).r
							? store_get(($$store_subs ??= {}), '$rGet', rGet)(d)
							: r),
					y: y2
				},
				target: {
					x:
						xMax +
						getOffset(xMax, offsetX, store_get(($$store_subs ??= {}), '$xScale', xScale)) -
						(store_get(($$store_subs ??= {}), '$config', config).r
							? store_get(($$store_subs ??= {}), '$rGet', rGet)(d)
							: r),
					y: y2
				...
				spread_props([
					{
						...
						fill:
							fill ??
							(store_get(($$store_subs ??= {}), '$config', config).c
								? store_get(($$store_subs ??= {}), '$cGet', cGet)(point.data)
								: null),
						fillOpacity,
						stroke,
						strokeWidth,
...
function Labels($$payload, $$props) {
	...
	getTextProps = (point) => {
		const pointValue = isScaleBand(store_get(($$store_subs ??= {}), '$yScale', yScale))
			? point.xValue
			: point.yValue;
		...
		const formattedValue = format(
			...
			format$12 ??
				(value
					? void 0
					: isScaleBand(store_get(($$store_subs ??= {}), '$yScale', yScale))
						? store_get(($$store_subs ??= {}), '$xScale', xScale).tickFormat?.()
						: store_get(($$store_subs ??= {}), '$yScale', yScale).tickFormat?.())
		);
...
function Tooltip($$payload, $$props) {
	...
	let tooltipWidth = 0;
	let tooltipHeight = 0;
	const xPos = motionStore(store_get(($$store_subs ??= {}), '$tooltip', tooltip).x, {
		spring: motion
	});
	const yPos = motionStore(store_get(($$store_subs ??= {}), '$tooltip', tooltip).y, {
		spring: motion
	});
	...
	if (store_get(($$store_subs ??= {}), '$tooltip', tooltip)?.data) {
		const xBandOffset = isScaleBand(store_get(($$store_subs ??= {}), '$xScale', xScale))
			? store_get(($$store_subs ??= {}), '$xScale', xScale).step() / 2 -
				(store_get(($$store_subs ??= {}), '$xScale', xScale).padding() *
					store_get(($$store_subs ??= {}), '$xScale', xScale).step()) /
					2
			: 0;
			...
			typeof x === 'number'
				...
				: x === 'data'
					? store_get(
							...
							'$xGet',
							xGet
						)(store_get(($$store_subs ??= {}), '$tooltip', tooltip).data) +
						store_get(($$store_subs ??= {}), '$padding', padding).left +
						xBandOffset
					: store_get(($$store_subs ??= {}), '$tooltip', tooltip).x;
		let xAlign = 'start';
		switch (anchor) {
			case 'top-left':
			case 'left':
		...
		const yBandOffset = isScaleBand(store_get(($$store_subs ??= {}), '$yScale', yScale))
			? store_get(($$store_subs ??= {}), '$yScale', yScale).step() / 2 -
				(store_get(($$store_subs ??= {}), '$yScale', yScale).padding() *
					store_get(($$store_subs ??= {}), '$yScale', yScale).step()) /
					2
			: 0;
			...
			typeof y === 'number'
				...
				: y === 'data'
					? store_get(
						...
						store_get(($$store_subs ??= {}), '$padding', padding).top +
						yBandOffset
					: store_get(($$store_subs ??= {}), '$tooltip', tooltip).y;
		let yAlign = 'start';
		switch (anchor) {
			case 'top-left':
			case 'top':
		...
		if (contained === 'container') {
			if (typeof x !== 'number') {
				if (
					...
					rect.right > store_get(($$store_subs ??= {}), '$containerWidth', containerWidth)
				) {
					rect.left = alignValue(xValue, 'end', xOffset, tooltipWidth);
				}
				if (
					(xAlign === 'end' || xAlign === 'center') &&
					rect.left < store_get(($$store_subs ??= {}), '$padding', padding).left
				) {
			...
			if (typeof y !== 'number') {
				if (
					...
					rect.bottom > store_get(($$store_subs ??= {}), '$containerHeight', containerHeight)
				) {
					rect.top = alignValue(yValue, 'end', yOffset, tooltipHeight);
				}
				if (
					(yAlign === 'end' || yAlign === 'center') &&
					rect.top < store_get(($$store_subs ??= {}), '$padding', padding).top
				) {
	...
	if (store_get(($$store_subs ??= {}), '$tooltip', tooltip).data) {
		...
		})}${attr('class', clsx(cls('absolute z-50 select-none', !pointerEvents && 'pointer-events-none', classes.root)))}><div${attr(
			'class',
			clsx(
				cls(
...
function AreaChart($$payload, $$props) {
	...
	visibleSeries = series.filter((s) => {
		return (
			// @ts-expect-error
			store_get(($$store_subs ??= {}), '$selectedSeries', selectedSeries).selected.length === 0 ||
			store_get(($$store_subs ??= {}), '$selectedSeries', selectedSeries).isSelected(s.key)
		);
	});
	...
	xScale =
		$$sanitized_props.xScale ??
		(accessor(x)(chartData[0]) instanceof Date ? scaleTime() : scaleLinear());
	getAreaProps = (s, i) => {
		const lineProps = {
			...props.line,
		...
		Chart(
			...
			spread_props([
				...
				$$restProps,
				{
					...
					brush:
						brush && (brush === true || brush.mode == void 0 || brush.mode === 'integrated')
							? {
									axis: 'x',
									resetOnEnd: true,
									xDomain,
					...
					$$slots: {
						default: (
							$$payload3,
							{ x: x2, xScale: xScale2, y: y2, yScale, c, cScale, width, height, padding, tooltip }
						) => {
							const slotProps = {
								x: x2,
								xScale: xScale2,
															...
															() => {
																...
																for (let i = 0, $$length = each_array_2.length; i < $$length; i++) {
																	...
																	const seriesTooltipData =
																		s.data && tooltip.data
																			? findRelatedData(s.data, tooltip.data, x2)
																			: null;
																	const highlightPointsProps =
																		typeof props.highlight?.points === 'object'
												...
												typeof legend === 'object' ? legend : null,
												{
													classes: {
														item: (item) =>
															...
															!visibleSeries.some((s) => s.key === item.value)
																? 'opacity-50'
																: '',
											...
											props.tooltip?.root,
											{
												...
												$$slots: {
													default: ($$payload4, { data: data2 }) => {
														TooltipHeader(
															$$payload4,
															spread_props([{ value: x2(data2), format }, props.tooltip?.header])
														);
														$$payload4.out += `<!----> `;
														TooltipList(
															$$payload4,
																	...
																	children: ($$payload5) => {
																		...
																		for (
																			...
																			const seriesTooltipData = s.data
																				? findRelatedData(s.data, data2, x2)
																				: data2;
																			const valueAccessor = accessor(
																				s.value ?? (s.data ? asAny(y2) : s.key)
																			);
																			TooltipItem(
																				$$payload5,
																				spread_props([
																					{
																						label:
																							s.label ?? (s.key !== 'default' ? s.key : 'value'),
																						value: seriesTooltipData
																							? valueAccessor(seriesTooltipData)
																							: null,
																						color: s.color,
																						format,
																						valueAlign: 'right',
																		...
																		if (stackSeries && visibleSeries.length > 1) {
																			...
																			TooltipSeparator(
																				$$payload5,
																				spread_props([props.tooltip?.separator])
																			);
																			$$payload5.out += `<!----> `;
																			TooltipItem(
																				$$payload5,
...
function _page($$payload, $$props) {
	...
	let percentCorrect = isNaN(+((correctCount / countComplete) * 100).toFixed(1))
		? 100
	...
	let visiblePercentCorrect = isNaN(
...
${rows}`;
	...
	function calculateAverageTimeDifference(items = filteredHistory) {
		...
		const totalMilliseconds = items.reduce((sum2, item) => {
	...
	Progressbar($$payload, {
		...
		size: 'h-6',
		labelInsideClass:
			'bg-green-600 text-green-100 text-base font-medium text-center p-1 leading-none rounded-full',
		class: 'mb-8'
	});
